...
/** When the lumbermill prefab completes, move Seren from the inn pool to the WORK POI. Quest completes on dialogue. */
public final class LumbermillCompletion {
    private static final HytaleLogger LOGGER = HytaleLogger.forEnclosingClass();

    private LumbermillCompletion() {}

    public static void onLumbermillBuilt(
        @Nonnull World world,
        ...
        if (work == null) {
            LOGGER.atWarning().log("No WORK POI for lumbermill plot %s", plotId);
            return;
        }
        ...
        Ref<EntityStore> npcRef = findLoggerRef(store, town);
        ...
        store.putComponent(
            ...
            new TownVillagerBinding(town.getTownId(), TownVillagerBinding.KIND_LOGGER, plotId, plotId)
        );
        town.addInnVisitorPoolExcludedRoleId(AetherhavenConstants.NPC_LOGGER);
        if (uuidComp != null) {
            ResidentRegistryService.upsert(
                town,
                tm,
                AetherhavenConstants.NPC_LOGGER,
        ...
        tm.updateTown(town);
        LOGGER.atInfo().log("Moved logger to lumbermill at %s,%s,%s", work.getX(), work.getY(), work.getZ());
    }

    @Nullable
    private static Ref<EntityStore> findLoggerRef(@Nonnull Store<EntityStore> store, @Nonnull TownRecord town) {
        ...
        for (String sid : ids) {
            try {
                ...
                if (npc != null && AetherhavenConstants.NPC_LOGGER.equals(npc.getRoleName())) {
                    return ref;
                }
